...
describe("Cypress Tests repeated from React assignment", () => {
  ...
  it("Acceptance Test: View question title containing search text", () => {
    const searchText = "React Router"; // Text to search for
    const expectedQuestionTitle = "Programmatically navigate using React router"; // Expected question title containing the search text
  
    // Visit the page and search for the text
    cy.visit("http://localhost:3000");
    cy.get("#searchBar").type(`${searchText}{enter}`);
  
    // Verify that the expected question title is displayed
    cy.get(".postTitle").should("contain", expectedQuestionTitle);
  });
});
