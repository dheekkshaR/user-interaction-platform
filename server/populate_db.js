...
function answerCreate(text, ans_by, ans_date_time, upvotes = [], downvotes = []) {
  ...
  if (upvotes) answerdetail.upvotes = upvotes;
  if (downvotes) answerdetail.downvotes = downvotes;
  let answer = new Answer(answerdetail);
  return answer.save();
}

function questionCreate(title, text, tags, answers, asked_by, ask_date_time, views, upvotes = [], downvotes = [], flagged = 0) {
  let qstndetail = {
    title: title,
    text: text,
    tags: tags,
...
function userCreate(name, username, typeOfUser, password, age, bio) {
  let user = new User({ name: name, username: username, typeOfUser: typeOfUser, password: password, age: age, bio: bio });
  return user.save();
}
...
const populate = async () => {
  // Mock user details
  let user1 = await userCreate('Alice', 'alice123', 'admin', 'password123', 25, 'Lorem ipsum dolor sit amet');
  let user2 = await userCreate('Bob', 'bob456', 'regular', 'pass456', 30, 'Consectetur adipiscing elit');
  let user3 = await userCreate('Charlie', 'charlie789', 'moderator', 'secure789', 28, 'Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua');
  let user4 = await userCreate('David', 'david987', 'regular', 'pass123', 35, 'Ut enim ad minim veniam');
  let user5 = await userCreate('Eve', 'eve111', 'admin', 'password567', 22, 'Quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat');
  ...
  let a2 = await answerCreate('On my end, I like to have a single history object that I can carry even outside components.', user2, new Date('2023-11-23T08:24:00'), [user3], [user4 ]);
  let a3 = await answerCreate('Consider using apply() instead; commit writes its data to persistent storage immediately.', user3 , new Date('2023-11-18T09:24:00'), [], [user1 ]);
  let a4 = await answerCreate('YourPreference yourPreference.saveData(YOUR_KEY,YOUR_VALUE);', user4 , new Date('2023-11-12T03:30:00'), [user5 ], []);
  let a5 = await answerCreate('I just found all the above examples just too confusing, so I wrote my own.', user5 , new Date('2023-11-01T15:24:19'), [user1 ], []);
  let a6 = await answerCreate('Storing content as BLOBs in databases.', user1 , new Date('2023-02-19T18:20:59'), [user2 ], []);
  let a7 = await answerCreate('Using GridFS to chunk and store content.', user2 , new Date('2023-02-22T17:19:00'), [user3 ], []);
  let a8 = await answerCreate('Store data in a SQLLite database.', user3 , new Date('2023-03-22T21:17:53'), [user4 ], []);

  // Mock questions
  await questionCreate('Programmatically navigate using React router', 'the alert shows the proper index for the li clicked.', [t1 , t2 ], [a1 , a2 ], user1 , new Date('2022-01-20T03:00:00'), 10, [user2 ], [user3 ], 1);
  await questionCreate('android studio save string shared preference', 'I am using bottom navigation view.', [t3 , t4 , t2 ], [a3 , a4 , a5 ], user2 , new Date('2023-01-10T11:24:30'), 121, [user3 ], [user4 ], 0);
  await questionCreate('Object storage for a web application', 'I am currently working on a website where, roughly 40 million documents and images should be served.', [t5 , t6 ], [a6 , a7 ], user3 , new Date('2023-02-18T01:02:15'), 200, [user4 ], [user5 ], 0);
  await questionCreate('Quick question about storage on android', 'I would like to know the best way to go about storing an array on an android phone.', [t3 , t4 , t5 ], [a8 ], user4 , new Date('2023-03-10T14:28:01'), 103, [user5 ], [user1 ], 0);
